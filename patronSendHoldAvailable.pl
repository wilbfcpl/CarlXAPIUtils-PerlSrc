...
#
# SOAPUI tool can provide a sandbox for the WSDL file and Puse constant SEND_HOLD_AVAILABLE => 'true' ;atronAPI requests.
...
%PatronUpdateValues =
    ( SendHoldAvailableFlag =>  defined($opt_x) ? 'true' : 'false'       
    );
