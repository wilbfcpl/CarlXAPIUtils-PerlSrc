...
# Version: 0.01
#
# Usage: perl [-d] AddNoteBouncedEmail.pl [-r ] [-x] [-g] filename.csv
...
use constant URGENT_EMAIL_NOTE_TYPE => 800;
use constant INFO_NOTE_TYPE => 501;

use constant BOUNCETYPE_SOFT_BOUNCE => 'S';
use constant BOUNCETYPE_HARD_BOUNCE => 'H';

use constant BOUNCED_EMAIL_SOFT_NOTE_TEXT => "Soft Email Bounce";
use constant BOUNCED_EMAIL_HARD_NOTE_TEXT => "Hard Email Bounce";
...
#  ($patronid, $noteid, $name, $btype, $notedate, $notetype,$branch,$actdate, $lastbranch, $alias)=split(/,/);
  ...
  ($patronid,$bouncetype)=split(/,/);  
   ...
   %AddNote =
	      ...
	      NoteText => ($bouncetype eq BOUNCETYPE_SOFT_BOUNCE ) ? BOUNCED_EMAIL_SOFT_NOTE_TEXT:BOUNCED_EMAIL_HARD_NOTE_TEXT
	   ) ;
